...
### Load data
### Add more data so we can group them
...
' 
lengthViolinPlot = ggplot(data = Eukdataframe, mapping = aes(x = OG, y = Length)) + geom_violin()
lengthViolinPlot
...
lengthViolinPlot = ggplot(data = SPO11dataframe, mapping = aes(x = OG, y = Length, fill = OG)) + geom_violin() + 
  labs(fill = "Domain", title = "Gene Length Box Plot", y = "Length of Sequence (bp)", x = "Name of Gene")

lengthViolinPlot + scale_fill_manual(values = c("#E0BBE4", "#FEC8D8"))
...
'
lengthViolinPlot + scale_x_discrete(limits=c("SPO11Prok"))

lengthViolinPlot + scale_x_discrete(limits=c("SPO11Euk"))

'
6. You can also use this to change the order in which the graphs appear
'
lengthViolinPlot + scale_x_discrete(limits=c("SPO11Prok", "SPO11Euk"))
...
'
### TYPE HERE ###
lengthViolinPlot + geom_boxplot(width=0.1) + geom_jitter(shape = 18, position = position_jitter(0.1)) + scale_fill_manual(values = c("#E0BBE4", "#FEC8D8"))
